  ...
  for(unsigned i = 0; i < cond.size(); i++) {
    ...
    for (unsigned j = 0; j < glb_names.size(); j++) {
      ...
      for( auto glb_idx_pair : b.m_model.ind_in_mem_state ) {
	    ...
	    if (mem_st[idx].t.type.fpa_ebits() == 8) {
	    ...
	    if (mem_st[idx].t.type.fpa_ebits() == 11) {
            precond_declarations.push_back(ctx.fpa_const(init_name.c_str(),11,53));
            std::string init_name1 = init_name + ".";
            precond_var_names.push_back(init_name1);
            precond_declarations.push_back(ctx.fpa_const(init_name1.c_str(),11,53));
            }
	  } 
  ...
  for(unsigned i = 0; i < cond.size(); i++) {
    ...
    for (unsigned j = 0; j < glb_names.size(); j++) {
	 ...
	 if (find(postcond_var_names.begin(), postcond_var_names.end(), init_name) == postcond_var_names.end()) {
	   ...
	   if (mem_st[idx].t.type.fpa_ebits() == 11) {
            postcond_declarations.push_back(ctx.fpa_const(init_name.c_str(),11,53));
            std::string init_name1 = init_name + ".";
            postcond_var_names.push_back(init_name1);
            postcond_declarations.push_back(ctx.fpa_const(init_name1.c_str(),11,53));
	   }
