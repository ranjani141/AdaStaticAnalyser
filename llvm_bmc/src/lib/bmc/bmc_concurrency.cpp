...
bool bmc_concur_pass::runOnFunction( llvm::Function &f ) {
  ...
  //for (unsigned l = 0; l < bmc_obj.sys_spec.threads.at(j).period; l++) {
    ...
    //std::cout << "Constraints for function : " << bmc_obj.sys_spec.threads.at(j).entry_function << " Thread " << bmc_obj.sys_spec.threads.at(j).name << " Thread num " << bmc_obj.sys_spec.threads.at(j).thread_num << " No. of posts " << bmc_obj.sys_spec.threads.at(j).posts.size() << "\n";
